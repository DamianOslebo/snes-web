...
const DEFS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      'Compile the 64 KB VRAM image (palette + tiles + tilemap). With `destName`, also register it on the ' +
      'assembler page as an `.incbin` data file — the standard name is "vram.bin" (then `.incbin "vram.bin"` ' +
      'in the program and DMA it to PPU VRAM).',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    run: (a, c) => {
      ...
      if (dn.v) {
        c.asm.addDataFile(dn.v, bytes);
        c.asm.appendSource(
          '\n; --- SPC700 loader glue (generated by trk_export_spc; EXPERIMENTAL) ---\n' +
            c.track.spcGlue(dn.v) +
            '\n',
        );
