...
/**
 * The in-app debugger. Fills the `#debug` slot with four panels:
 ...
 *   4. Breakpoints / Save states (capture, restore, diff)
 *
 * The heavy lifting lives in the pure modules (./disasm, ./memory-view,
 * ./savestate); this file is only DOM + wiring, so it returns a small
 * `DebugHandle` the rest of the app can drive (main.ts uses it for the
 * transport buttons and the breakpoint callback).
 */
