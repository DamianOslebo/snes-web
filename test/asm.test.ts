...
describe('65C816 assembler', () => {
  it('encodes every standard addressing form', () => {
    const r = expectOk([
      ...
      'LDA $0010,Y',   // bd 10 00    (65C816 has no zp,Y)
      'LDA $0010,X',   // b9 10 00    absolute,X
      'ORA ($0A),Y',   // 01 0a       indexed-indirect,Y
      'ORA ($0A,X)',   // 11 0a       indexed-indirect
      ...
      'RTS',           // 40          (65C816: RTS=$40, RTI=$60)
    ].join('\n'));
    expect(hexBytes(r)).toBe(
      'a9 2a 8d 4c 00 a5 10 b5 10 bd 10 00 b9 10 00 01 0a 11 0a 6c 80 c0 22 00 80 c0 20 00 01 40');
  });
  ...
  it('classifies numeric addresses by the width as written', () => {
    ...
    expect(hexBytes(expectOk('LDA $000A,X'))).toBe('b9 0a 00'); // 4 digits → abs,X (B9)
  });
  ...
  it('resolves labels forward and backward', () => {
    ...
    expect(hexBytes(r)).toBe('a9 01 f0 01 e8 40 c6 0a d0 fc');
  ...
  it('resolves labels that follow 8-bit immediates (fixed-point seed)', () => {
    ...
    expect(hexBytes(r)).toBe('a9 05 f0 01 ea 40');
  ...
  it('resolves labels that follow 16-bit immediates', () => {
    ...
    expect(hexBytes(r)).toBe('c2 01 a9 34 12 f0 01 ea 40');
  ...
  it('widens immediates after REP #$1 and narrows after SEP #$1', () => {
    ...
    expect(hexBytes(r)).toBe('c2 01 a9 ff ff e2 01 a9 ff 40');
  ...
  it('rejects malformed sources with a precise message per case', () => {
    ...
    expectFail('LDX $0A,Y', /absolute,Y/);
    expectFail('LDA ($0100)', /indirect/);   // 65C816 has no LDA (abs)
